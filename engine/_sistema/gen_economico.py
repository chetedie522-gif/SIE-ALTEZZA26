...
a.back{color:var(--acc);text-decoration:none;font-size:13px;display:inline-block;margin-bottom:8px}
...
HEAD_BODY = ('<a class="back" href="../">← volver al hub</a>'
 '<header><h1 id="obra">-</h1><div class="sub" id="meta">cargando...</div></header>'
 '<div class="conf">CONFIDENCIAL · Solo direccion — contiene costos y margenes. No compartir como el tablero publico.</div>'
 '<div id="app"></div>'
 '<footer>SIE ALTEZZA26 · Control economico. Uso interno de direccion.</footer>')

full = ('<!DOCTYPE html>\n<html lang="es"><head><meta charset="utf-8">'
 '<meta name="viewport" content="width=device-width, initial-scale=1">'
 '<title>Control economico ALTEZZA26</title><style>' + CSS + '</style></head><body>'
 + HEAD_BODY +
 '<script>\nconst DATA = ' + DATA + ';\n' + JS +
...
art = ('<title>Control economico ALTEZZA26</title><style>' + CSS + '</style>'
 + HEAD_BODY +
 '<script>\nconst DATA = ' + DATA + ';\n' + JS + '\nrender(DATA);\n</script>')
sp = os.path.join(SYS, "economico_altezza26.html")
open(sp, "w", encoding="utf-8").write(art)
print("OK -> index.html (OneDrive) y economico_altezza26.html (artifact)")
